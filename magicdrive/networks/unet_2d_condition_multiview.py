        ...
        for module in self.modules():
            if isinstance(module, (CrossAttnDownBlock2D, DownBlock2D, CrossAttnUpBlock2D, UpBlock2D)):
                ...
                if flag is not None and not flag[mod_idx]:
                    ...
                    continue
                logging.debug(f"[{self.__class__.__name__}] set "
    ...
    def from_unet_2d_condition(
        ...
        **kwargs,
        ...
        for k in unet.config.keys():
            if k not in kwargs:
                _config[k] = unet.config[k]
            else:
                logging.info(f"[UNet]: You provide new {k} as {kwargs[k]}.")
        ...
        if load_weights_from_unet:
            ...
            logging.info(
                f"[{unet_2d_condition_multiview.__class__.__name__}] load "
                f"pretrained with missing_keys: {missing_keys}; "
