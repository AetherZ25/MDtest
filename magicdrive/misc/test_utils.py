...
def run_one_batch(cfg, pipe, val_input, weight_dtype, global_generator=None,
    ...
    camera_param_raw = move_to(move_to(
        val_input["camera_param_raw"], weight_dtype), pipe.device)

    # 3-dim list: B, Times, views
    gen_imgs_list = run_one_batch_pipe_func(
        cfg, pipe, val_input['pixel_values'], val_input['captions'],
        val_input['bev_map_with_aux'], camera_param, camera_param_raw,
        val_input['kwargs'], global_generator=global_generator)
