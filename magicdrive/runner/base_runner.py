...
class BaseRunner:
    def __init__(self, cfg, accelerator, train_set, val_set) -> None:
        ...
        self.foreground_loss_mode = self.cfg.runner.foreground_loss_mode
        self.foreground_loss_weight = self.cfg.runner.foreground_loss_weight
        ...
        for epoch in range(first_epoch, self.cfg.runner.num_train_epochs):
            for step, batch in enumerate(self.train_dataloader):
                ...
                if self.cfg.runner.save_model_per_epoch is not None:
                    _r_epoch = epoch + 1
                    if _r_epoch % self.cfg.runner.save_model_per_epoch == 0:
                        logging.info(
                            f"Save at step {global_step}, epoch {_r_epoch}")
                        self.accelerator.wait_for_everyone()
                        sub_dir_name = f"weight-E{_r_epoch}-S{global_step}"
    ...
    def _train_one_step(self, batch):
        ...
        with self.accelerator.accumulate(self.controlnet):
            ...
            controlnet_image = batch["bev_map_with_aux"].to(
                dtype=self.weight_dtype)
            camera_params_raw = move_to(
                batch["camera_param_raw"], self.weight_dtype)
